...
export default function UnifiedImportPage() {
  ...
  const loadAllData = async () => {
    ...
    const { data: dp } = await supabase.from('decoration_partners').select('*').order('name');
    if (dp) setDecorationPartners(dp);
    const { data: dc } = await supabase.from('decoration_catalog').select('*').order('name_en');
    ...
    <div style={{minHeight:'100vh',backgroundColor:'#f3f4f6',padding:'20px'}}>
      <div style={{maxWidth:'1400px',margin:'0 auto'}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'20px'}}>
          <div>
            <h1 style={{fontSize:'28px',fontWeight:'bold',color:'#1f2937'}}>📦 Импорт контрактов</h1>
            <p style={{color:'#6b7280',marginTop:'4px'}}>Загружайте прайсы партнёров и просматривайте данные</p>
          </div>
          <a href="/" style={{padding:'8px 16px',backgroundColor:'#e5e7eb',borderRadius:'6px',color:'#374151',textDecoration:'none'}}>← К калькулятору</a>
        </div>

        <div style={{display:'flex',backgroundColor:'white',borderRadius:'12px 12px 0 0'}}>
          {tabs.map(tab => (
            <button key={tab.type} onClick={() => { setActiveType(tab.type); setSelectedPartnerId(null); setParsedItems([]); setContractText(''); }}
              style={{flex:1,padding:'16px',border:'none',cursor:'pointer',fontSize:'14px',fontWeight:'500',display:'flex',alignItems:'center',justifyContent:'center',gap:'8px',
