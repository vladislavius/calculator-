  ...
  return (
    <div id="summary" style={{ padding: isMobile ? '12px' : '24px', background: 'linear-gradient(135deg, #1e40af 0%, #7c3aed 100%)', borderRadius: isMobile ? '10px' : '16px', color: 'white' }}>
        ...
        {s.markupMode === 'percent' ? (
          <>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '6px', flexWrap: 'wrap' }}>
              ...
              <span style={{ fontSize: isMobile ? '10px' : '13px', opacity: 0.7 }}>= +{Math.round((selectedBoat?.calculated_total || 0) * s.boatMarkup / 100).toLocaleString()} THB</span>
            </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
            ...
            <span style={{ fontSize: isMobile ? '10px' : '13px', opacity: 0.7 }}>= {((selectedBoat?.calculated_total || 0) > 0 ? (s.fixedMarkup / (selectedBoat?.calculated_total || 1) * 100).toFixed(1) : 0)}%</span>
          </div>
        )}
      </div>
      ...
      <div style={{ backgroundColor: 'rgba(255,255,255,0.1)', borderRadius: '12px', padding: isMobile ? '10px' : '20px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', padding: isMobile ? '5px 0' : '10px 0', borderBottom: '1px solid rgba(255,255,255,0.2)', fontSize: isMobile ? '12px' : '14px' }}>
          <span>Яхта</span>
          <span style={{ fontWeight: '600' }}>{(selectedBoat.calculated_total || 0).toLocaleString()}</span>
        </div>
        {guestSurcharge > 0 && (
          <div style={{ display: 'flex', justifyContent: 'space-between', padding: isMobile ? '5px 0' : '10px 0', borderBottom: '1px solid rgba(255,255,255,0.2)', fontSize: isMobile ? '12px' : '14px' }}>
            <span>Доп. гости</span>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', padding: isMobile ? '5px 0' : '10px 0', borderBottom: '1px solid rgba(255,255,255,0.2)', color: '#fcd34d', fontSize: isMobile ? '12px' : '14px' }}>
          <span>Наценка</span>
          <span style={{ fontWeight: '600' }}>+{s.markupMode === 'fixed' ? s.fixedMarkup.toLocaleString() : Math.round((selectedBoat.calculated_total || 0) * s.boatMarkup / 100).toLocaleString()}</span>
        </div>
